...
const Profile = (props) => {
  ...
  const { loading, error, data } = useQuery(getProfileInfoQuery, {
    variables: { id },
  });
  ...
  const uploadHandler = async (e) => {
    ...
    fetch("/upload/", {
      ...
      .catch((err) =>
        setError(
          "Something went wrong when trying to update the profile image, please try again"
        )
      );
  };

  useEffect(() => {}, [isUploaded]);
  ...
  return (
    <>
      <div className='profile'>
        {AllowEdit ? (
          <button onClick={() => setIsOpen(true)} className='edit-profile-btn'>
            <i class='far fa-edit'></i>
          </button>
        ) : null}
        <div className='profile-img'>
          <img
            src={`../${data.profile.user._id}.jpg`}
            onError={(e) => (e.target.src = "../account.jpg")}
            alt='profile'
          />
          {AllowEdit ? (
            <div>
              <label for='upload-image' className='upload-image'>
        ...
        <div className='profile-name'>
          ...
          <span>{`joined at ${modifyDate(
            Number(data.profile.user.createdAt)
          )}`}</span>
        </div>
