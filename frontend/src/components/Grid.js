...
const Grid = ({ project, tasks, errorHandler }) => {
  ...
  const options = ["bucket", "start", "end", "dependsOn", "dependants"].filter(
    (opt) => {
      return !columns.includes(opt);
    }
  );
  ...
  const addTaskNode = useClickToClose(
  ...
  const addColNode = useClickToClose(
    () => hideAndShow("select-col", "add-col"),
    "#select-col"
  );
        ...
        <div className='grid-header'>
          ...
          {columns &&
            columns.map((col) => (
              <span className='additional-col' key={col}>
                {col}{" "}
                <i
                  onClick={(e) => handleHideColumn(e, col)}
          ...
          {options.length > 0 ? (
            <div className='add-col-container'>
              <select ref={addColNode} id='select-col' className='hide'>
                {options.map((opt, indx) => (
                  <option
                    key={indx}
                    onClick={(e) => handleAddColumn(e.target.value)}
                  >
                    {opt}
                  </option>
                ))}
              </select>
              <button id='add-col' onClick={() => handleShowSelect()}>
                Add column
              </button>
            </div>
          ) : null}
