...
const AdditionalColumns = ({ columns, task }) => {
  ...
  const hideShowDependacy = () => {
    const container = document.querySelector(
      ".grid-dependacy-container:not(.hide)"
    );
    if (container) {
      container.classList.add("hide");
    }
  };
  const dependacynode = useClickToClose(
    hideShowDependacy,
    ".grid-dependacy-container:not(.hide)"
  );
        ...
        columns.map((col, index) =>
          col === "dependsOn" || col === "dependants" ? (
            <div key={index} className='dependacy-row'>
              {col === "dependsOn" && task.dependsOn.length > 0 ? (
                <button
                  className='dependacy-btn'
                  onClick={(e) => handleShowDependacy(e)}
                >
                  ({task.dependsOn.length}) tasks
                  <div
                    ref={dependacynode}
                    id='dependacy-container'
                    className='grid-dependacy-container hide'
                  >
              ...
              {col === "dependants" && task.dependants.length > 0 ? (
                <button
                  className='dependacy-btn'
                  onClick={(e) => handleShowDependacy(e)}
                >
