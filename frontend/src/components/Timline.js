...
import { getDeffrenceInDays, modifyDate, addDays } from "../helpers/helpers";
      ...
      <div className='timeline'>
        ...
        <div className='timeline-side'>
          {project.tasks.map((task) => (
            <div key={task._id} className='timline-side-task'>
              {task.name}
            </div>
          ))}
        </div>
        <div className='timeline-main'>
          {project.tasks.map((task) => {
            ...
            const daysBeforStart = getDeffrenceInDays(
              task.start,
              modifyDate(project.createdAt)
            );

            return (
              <div
                className='timline-task'
                style={{ width: duration * 20 + "px" }}
              >
                <Bar
                  length={days}
                  befor={daysBeforStart}
                  task={task}
