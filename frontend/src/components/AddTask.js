      ...
      <div ref={node} className='add-task'>
        {isAllowedGeneral && (
          <div className='single assign-users'>
            <button onClick={(e) => showGroupSearch(e)}>
            ...
            <GroupSearch
              group={filterGroup(project.group, task.assignedTo)}
              type='add'
              position={"group-search-containerv2"}
              domNode={domNode}
              taskID={taskID}
              projectID={project._id}
            />
        ...
        <div className='single'>
          <label>Name</label>
          <input
            onChange={(e) => updateTaskHandler({ name: e.target.value })}
        ...
        <div className='double'>
          <div>
            ...
            <input
              ...
              max={
                task.end
                  ? removeDays(task.end, 1).toISOString().split("T")[0]
                  : null
              }
          ...
          <div>
            ...
            <input
              min={
                task.start
                  ? addDays(task.start, 1).toISOString().split("T")[0]
                  : null
              }
        ...
        <div className='double'>
          <div>
            ...
            <input
              onChange={(e) =>
                updateTaskHandler({ duration: Number(e.target.value) })
              }
              readOnly={true}
              defaultValue={
                task.start && task.end
          ...
          <div>
            ...
            <input
              onChange={(e) =>
                updateTaskHandler({ completion: Number(e.target.value) })
              }
              readOnly={!isAllowed}
          ...
          <select disabled={!isAllowed} defaultValue={task.bucket}>
            ...
            {project.buckets.map((bucket, index) => (
              <option
                key={index}
                onClickCapture={(e) =>
                  updateTaskHandler({ bucket: e.target.value })
                }
              >
                {bucket}
              </option>
        ...
        <div className='trible'>
          <div>
            ...
            <input
              onChange={(e) =>
                updateTaskHandler({ totalEffort: Number(e.target.value) })
              }
              readOnly={!isAllowed}
              defaultValue={task.totalEffort}
              type='number'
            />
          </div>
          <div>
            ...
            <input
              onChange={(e) =>
                updateTaskHandler({ doneEffort: Number(e.target.value) })
              }
              readOnly={!isAllowed}
              defaultValue={task.doneEffort}
              type='number'
            />
          </div>
          <div>
            ...
            <input
              defaultValue={task.totalEffort - task.doneEffort}
              type='number'
              readOnly
            />
          </div>
              ...
              <select defaultValue={task.dependsOn[0]}>
                {project.tasks.map((task) => (
                  <option
                    key={task._id}
                    onClickCapture={(e) =>
                      addDependacyHandler(task._id, "dependsOn")
                    }
                  >
                    {task.name}
                  </option>
              ...
              <select defaultValue={task.dependants[0]}>
                {project.tasks.map((task) => (
                  <option
                    key={task._id}
                    onClickCapture={(e) =>
                      addDependacyHandler(task._id, "dependants")
                    }
                  >
                    {task.name}
                  </option>
